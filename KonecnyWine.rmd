...
The majority of wines is of quality 5 to 7. There appears no clear patter linking suplhates to quality (and therefore rating). This is the case for most variables.
...
```{r echo=FALSE}
plot_bivar <- function(var1, var2, data1 = wines_w, alpha1 = 0.25){
  iqr1 <- IQR(var1)
  md1 <- median(var1)
  r1 <- c(md1- 2*iqr1, md1+ 2*iqr1)
  ...
  print(strsplit(deparse(substitute(var1)),"\\$")[[1]][2])
  print(strsplit(deparse(substitute(var2)),"\\$")[[1]][2])
  
  p1 <- ggplot(aes(x = alcohol , y = pH, color = rating),
                    data= data1)+
               geom_point(alpha = alpha1, position = position_jitter(h=0))+
               coord_cartesian(ylim = r1)+
               coord_cartesian(xlim = r2)+
               #only use the part after $
               ylab(strsplit(deparse(substitute(var1)),"\\$")[[1]][2])+
               xlab(strsplit(deparse(substitute(var2)),"\\$")[[1]][2])
...
# p1<-plot_vs_rating(wines_w$fixed.acidity)
# p2<-plot_vs_rating(wines_w$volatile.acidity)
# p3<-plot_vs_rating(wines_w$citric.acid)
# p4<-plot_vs_rating(wines_w$residual.sugar)
# p5<-plot_vs_rating(wines_w$chlorides)
# 
# p6<-plot_vs_rating(wines_w$free.sulfur.dioxide)
# p7<-plot_vs_rating(wines_w$total.sulfur.dioxide)
# p8<-plot_vs_rating(wines_w$ratio.sulfur.dioxide)
# p9<-plot_vs_rating(wines_w$density)
# p10<-plot_vs_rating(wines_w$pH)
# 
# p11<-plot_vs_rating(wines_w$sulphates)
# p12<-plot_vs_rating(wines_w$alcohol)
# 
...
```{r}
#plot_bivar(filtered_wines_w$volatile.acidity, filtered_wines_w$alcohol, data = filtered_wines_w)

#plot_bivar(wines_w$volatile.acidity, wines_w$alcohol, data = wines_w, alpha = 0.2)

#plot_vs_quality(wines_w$volatile.acidity, data = wines_w)

#explore_var(wines_w$volatile.acidity)

fw0 <- filter_data(quote(volatile.acidity < 0.26), data = filtered_wines_w)



table(filtered_wines_w$quality)/length(filtered_wines_w$quality)
table(fw0$quality)/length(fw0$quality)

# ggplot(aes(x = wines_w$alcohol , y = wines_w$volatile.acidity, color = rating), data= wines_w)+
