...
I started with plotting the variable pairings identified in the previous section. I have removed the outliers (lying outside of the IQR) and scaled the axis to focus on the majority of data points. I have also customized the alpha value decreasing alpha for medium (forming the overwhelming majority) and increasing the alpha for bad and excellent wines (which are rare).

The plot confirms the trends I already know, e.i. high alcohol and low density wines tend to have higher quality. However bad quality wines are also scattered over the plot.  
...
The plots are still very noisy and dominated by medium wines (green). Next I decided to focus only on filtered wines. I will repeat the same analysis as before. First I calculate Pearson's R between quality and other variables and quality, this time considering only the filtered wines.
...
There are very few similarities in the two lists. In both alcohol shows the strongest correlation with quality. For filtered wines the difference between alcohol and the second best variable (residual sugar) is very small.
...
In plot below size corresponds to alcohol and shape to pH. I have tried different combinations of variables, but I couldn't find a combination which reliably discriminates between good, medium and excellent wines. On the contrary, I have established that medium and excellent wines can be very similar  (plotted against a red background).
...
It was very interesting to see how the importance of features has changed when considering only the filtered wines. In hindsight, it was to be expected. When I removed many wines based on a particular feature, one could expect that this feature will become less important. 

Yet I found the extent of this surprising. Density and chlorides are the features least correlate with quality in the filtered dataset. Prior to fitlering, they were the most correlated features only (after alcohol). 

The plot below nicelly shows the difference between the two data sets. The plot for all wines shows a linear tendency, while the plot for filtetered wines is more scattered.
...
It is remarkable how strongly alcohol correlates with quality. I would be curious to know whether this is because of poor quality wines have little alcohol. Or becuase the high alcohol content numbs the finer distinctions beween wines.

### Were there any interesting or surprising interactions between features?

I was curious whether I can find a reliable way to tell apart medium, good and excellent wines. This is not the case. It was really interesting to see how very similar are two wines which significantly differ in quality.
...
Base on my investigation I am sceptical about finding an objective and universal  relationship between the measured variables and assigned quality. 
