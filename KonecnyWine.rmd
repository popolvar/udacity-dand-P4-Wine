...
```{r echo=FALSE}
fw<-filter_data(quote(chlorides<0.037))
fw<-filter_data(quote(ratio.sulfur.dioxide>0.2717), data = fw)
p1 <- quality_bar("Chlorides+RatioSulfur", fw)

fw <-filter_data(quote(alcohol>12.4))
p2 <- quality_bar("Alcohol", fw)

fw<-filter_data(quote(density<0.9918), data = fw)
p3 <- quality_bar("Alc+Density", fw)

fw<-filter_data(quote(chlorides<0.037), data = fw)
fw<-filter_data(quote(ratio.sulfur.dioxide>0.2717), data = fw)

p4 <- quality_bar("All", fw)
p4 <- p4+scale_fill_manual(values = used_cols[2:4])
...
I started with plotting the variable pairings identified in the previous section. I have removed the outliers (lying outside of IQR) and scalled the axis to focus on the majority of data points. I have also customized the alpha value decreasing alpha for medium (forming the overwhelming majority) and increasing the alpha for rare bad and excellent wines.

The plot confirm the trends I already know, e.i. high alcohol and low density wines tend to have higher quality. However bad quality wines are also scattered over the plot.  
...
```{r echo=FALSE}
plot_bivar(wines_w$alcohol, wines_w$density)
...
#cor(filtered_wines_w[4],filtered_wines_w[,c(2:12,15)])
```

I will later also calculate the corellation between these variable to identifies suitable pairings for multivariete plotting. It would be interesting  to pick two variables with strong negative correlation.

```{r echo=FALSE}
mn <- names(cor_f[,abs(cor_f) > 0.2])
cor(filtered_wines_w[, mn])

plot_bivar(filtered_wines_w$residual.sugar, filtered_wines_w$total.sulfur.dioxide,
           data = filtered_wines_w)
plot_bivar(filtered_wines_w$residual.sugar, filtered_wines_w$alcohol,
           data = filtered_wines_w)
plot_bivar(filtered_wines_w$alcohol, filtered_wines_w$total.sulfur.dioxide,
           data = filtered_wines_w)

plot_bivar(filtered_wines_w$citric.acid, filtered_wines_w$volatile.acidity,
           data = filtered_wines_w)+
  coord_cartesian(xlim = c(0.1,0.6))
```
