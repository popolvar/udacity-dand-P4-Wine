...
wines_r <-read.csv('wineQualityReds.csv')
str(wines_r)

table(wines_r$quality)
```
...
This data contains information about quality of (Vinho Verde) white wines from Minho. It contains objective meassurment of 11 chemical attributes of 4898 different white wines along with a quality score.

The quality score is median value from and expert jury with at least three judges. It would be very interesting to have also the scores of individual judges, so I could see the variance in scores. Unfortuantelly only the aggregate value is provided.

Quality takes a numerical value, but it is effectivelly an ordered factor. According to [dataset description](https://s3.amazonaws.com/udacity-hosted-downloads/ud651/wineQualityInfo.txt) possible values for quality range from 0 (very bad) and 10 (very excellent). The [academic paper](https://www.researchgate.net/publication/222430341_Modeling_wine_preferences_by_data_mining_from_physicochemical_properties) referencing the dataset suggests that expert judged the wine on numeric scale.
...
wines_w$rating = ordered(cut(wines_w$rating,c(0,5,6,7,8), 
