...
wines_w$ratio.sulfur.dioxide<-wines_w$free.sulfur.dioxide/
  wines_w$total.sulfur.dioxide
```
...
#show_all_hist shows dashed histogram of the original dataset 
# (used in conjunction with filtered datasets)

quality_bar<-function(title, data1 = wines_w, show_leg = FALSE,
                      show_all_hist = FALSE){
  p1<-ggplot(aes(x = as.factor(quality), fill = rating), data = data1) +
  geom_bar() +
  xlab('Quality') +
  ylab('Count')+
  ggtitle(title)+
  ...
  if(show_all_hist){
    p1<-p1+geom_bar(color = 'red', fill = NA, linetype="dashed", 
                    data = wines_w)
  }
...
p1 <-quality_bar("Quality distribution", show_all_hist = FALSE)
p1
```
...
ggplot(aes(x = alcohol), data = wines_w) +
  geom_histogram(color = 'gray', fill = 'orange', bins = 60)+
  theme_dark()

summary(wines_w$alcohol)
...
explore_var <- function(var1, data1 = wines_w){
  ...
  var_name <-strsplit(deparse(substitute(var1)),"\\$")[[1]][2]
  ...
  xlab(paste(var_name,"(no outliers)",sep=""))+
  scale_fill_brewer(palette = "Blues")+
  theme_dark()
  #scale_x_log10()
  
  # boxplot
  b1<-ggplot(aes(y = var1, x= rating, color = rating), data = data1)+
  ...
  #for all variables
  h0<-ggplot(aes(x = var1), data = wines_w)+
  geom_histogram(color = 'gray', fill = 'orange', bins = 60)+
  #xlab(paste("log(",var_name,")",sep=""))+
  xlab(paste(var_name,"(no outliers)",sep=""))+
  scale_fill_brewer(palette = "Blues")+
...
res_var[[4]]+coord_cartesian(xlim = c(0.0,0.13))
res_var[[5]]
```

Most variables follow a similar trend. To handle outliers to the right better I will use a logaritmic scale on the x axis.

```{r, echo=FALSE}
res_var <-explore_var(wines_w$citric.acid)
res_var[[4]]+coord_cartesian(xlim = c(0.0,1.0))
res_var[[5]]
```

```{r, echo=FALSE}
res_var <-explore_var(wines_w$volatile.acidity)
res_var[[4]]+coord_cartesian(xlim = c(0.0,0.75))
res_var[[5]]
```

```{r, echo=FALSE}
res_var <-explore_var(wines_w$density)
res_var[[4]]+coord_cartesian(xlim = c(0.98,1.01))
res_var[[5]]
```

```{r, echo=FALSE}
res_var <-explore_var(wines_w$free.sulfur.dioxide)
res_var[[4]]+coord_cartesian(xlim = c(0.0,125))
res_var[[5]]
```

```{r, echo=FALSE}
res_var <-explore_var(wines_w$total.sulfur.dioxide)
res_var[[4]]+coord_cartesian(xlim = c(0,350))
res_var[[5]]
```

```{r, echo=FALSE}
res_var <-explore_var(wines_w$ratio.sulfur.dioxide)
res_var[[4]]
res_var[[5]]
```
...
```{r, echo=FALSE}
res_var <-explore_var(wines_w$pH)
res_var[[4]]
res_var[[5]]
```
...
```{r, echo=FALSE}
res_var <-explore_var(wines_w$alcohol)
res_var[[4]]
res_var[[5]]
```

Alcohol doesn't look resemble a normal distribution.

```{r, echo=FALSE}
res_var <-explore_var(wines_w$residual.sugar)
res_var[[4]]+coord_cartesian(xlim = c(0,30))
res_var[[5]]
```

Residual sugar has clearly two distinct peaks.

```{r, echo=FALSE}
res_var <-explore_var(wines_w$sulphates)
res_var[[4]]
res_var[[5]]
```

Sulphates appear to follow a normal distribution although there are many 'spikes' disturbing the smooth curve.
...
```{r echo=FALSE}
res_var[[2]]+coord_cartesian(ylim = c(0,0.1))
```
...
filtered_wines_w<-filter_data(quote(ratio.sulfur.dioxide>0.2717),
...
filtered_wines_w <-filter_data(quote(density<0.9918),
                               data = filtered_wines_w)
...
#(table(wines_w$quality) - table(filtered_wines_w$quality))
#/table(wines_w$quality)
```
...
explore_filtering <- function(var1, data1 = baf_wines_w, f1 = 'filtered'){
  ...
  p1<-ggplot(aes(y = var1, x= filtering, color= rating),
             data = data1)+
    geom_boxplot(alpha=0.5)+
    #only use the part after $
    ylab(var_name)+
  ...
  p2<-ggplot(aes(y = var1, x= rating, color= rating),
             data = data1)+
    geom_boxplot(alpha=0.5)+
    #only use the part after $
    ylab(var_name)+
  ...
  p3<-ggplot(aes(y = var1, x= rating, color= rating),
             data = data1)+
    geom_boxplot(alpha=0.5)+
    #only use the part after $
    ylab(var_name)+
...
p1 <- ggplot(aes(x = as.factor(quality), fill = rating), data = fw1) +
  geom_bar() +
  xlab('Quality') +
  ylab('Count')+
  ggtitle("No C.A.")+
...
p0 <- ggplot(aes(x = as.factor(quality), fill = rating), data = fw0) +
  geom_bar() +
  xlab('Quality') +
  ylab('Count')+
  ggtitle("All")+
...
#filter 
#filtered2_wines_w<-filter_data(quote(residual.sugar > 2.6), 
#                               data = filtered_wines_w)

fw <- filter_data(quote(residual.sugar > 2.6), data = filtered_wines_w)
p1 <- quality_bar("Residual Sugar", fw)
p1 <- p1+geom_bar(color = 'green', fill = NA, linetype="dashed",
                  data = filtered_wines_w)

filtered2_wines_w<-filter_data(quote(residual.sugar > 4.212), 
                               data = filtered_wines_w)
fw <- filter_data(quote(residual.sugar > 4.212), data = filtered_wines_w)
p2 <- quality_bar("Residual Sugar", fw)
p2 <- p2+geom_bar(color = 'green', fill = NA, linetype="dashed",
                  data = filtered_wines_w)

#SULFUR
filtered2_wines_w<-filter_data(quote(total.sulfur.dioxide > 113.8), 
                               data = filtered2_wines_w)
#filtered2_wines_w<-filter_data(quote(total.sulfur.dioxide > 113.8),
#                               data = filtered_wines_w)
fw <- filter_data(quote(total.sulfur.dioxide > 113.8), 
                  data = filtered_wines_w)
p3 <- quality_bar("T. S. D.", fw)
p3 <- p3+geom_bar(color = 'green', fill = NA, linetype="dashed",
                  data = filtered_wines_w)

#CITRIC ACID
#filtered2_wines_w<-filter_data(quote(citric.acid > 0.3375),
#                               data = filtered2_wines_w)
fw <- filter_data(quote(citric.acid > 0.3375), data = filtered_wines_w)
p4 <- quality_bar("Citric Acid", fw)
p4 <- p4+geom_bar(color = 'green', fill = NA, linetype="dashed", 
                  data = filtered_wines_w)

#ALCOHOL
filtered2_wines_w<-filter_data(quote(alcohol > 12.40), 
                               data = filtered2_wines_w)
fw <- filter_data(quote(alcohol > 12.40), data = filtered_wines_w)
p5 <- quality_bar("Alcohol", fw)
p5 <- p5+geom_bar(color = 'green', fill = NA, linetype="dashed", 
                  data = filtered_wines_w)

p6 <- quality_bar("RS+CA", filtered2_wines_w)
#p6 <- p6+geom_bar(color = 'green', fill = NA, linetype="dashed",
#                  data = filtered_wines_w)
```
...
plot_vs_quality <- function(var1, data1 = wines_w, alpha1 = 0.25, 
                            show_leg = FALSE){
  ...
  p1 <- ggplot(aes(x = as.factor(quality), y = var1, color = rating),
                    data= data1)+
               geom_point(alpha = alpha1, position = position_jitter(h=0))+
               stat_summary(fun.y=mean, color = "red", geom="point",
                            size = 2, aes(fill = "mean"))+
               stat_summary(fun.y=median, color="orange", geom="point",
                            size=2, aes(fill = "2Q"))+
               stat_summary(fun.y=qwrap, fun.args = list(c(0.25)),
                            color="yellow", geom="point", size=2,
                            aes(fill = "1Q"))+
               stat_summary(fun.y=qwrap, fun.args = list(c(0.75)),
                            color="brown",geom="point", size=2,
                            aes(fill = "3Q"))+
               scale_fill_manual("statistics", values = rep(1,4),
                  guide=guide_legend(override.aes =
                                       list(colour = c("yellow", "orange",
                                                       "brown", "red"))))+
...
plot_vs_quality(wines_w$sulphates, show_leg = T)
```
...
```{r, echo=FALSE}
p1 <- plot_vs_quality(wines_w$volatile.acidity, show_leg = T)
...
```{r echo=FALSE}
exclude <- c("X", "quality", "rating","filtering")
keep <- !names(wines_w) %in% exclude
cor_all <-cor(wines_w["quality"],wines_w[,keep])
...
```{r echo=FALSE}
plot_bivar <- function(var1, var2, data1 = wines_w, show_leg = FALSE,
  ...
  p1 <- ggplot(aes(x = var1 , y = var2),
               ...
               scale_color_brewer(palette = "Blues")+
               theme_dark()+
               geom_smooth(aes(color = rating), method='lm',
                           se = F, size =1)
...
```{r echo=FALSE}
plot_bivar(wines_w$alcohol, wines_w$density, remove_outliers = T,
           show_leg = T)#+
...
plot_bivar(filtered_wines_w$residual.sugar, 
           filtered_wines_w$total.sulfur.dioxide,
           data = filtered_wines_w,
           show_leg = T)
...
plot_bivar(filtered_wines_w$alcohol, filtered_wines_w$total.sulfur.dioxide,
           data = filtered_wines_w, show_leg = T)
```
...
```{r echo=FALSE}
p0 <- plot_bivar(wines_w$alcohol, wines_w$density, show_leg = T)
...
p1 <- plot_vs_quality(wines_w$chlorides)
p1 <- p1 + geom_hline(yintercept = 0.037, linetype="dotted", color = "blue")
p1 <- p1 + ylab("chlorides [g / dm^3]")

p2 <- plot_vs_quality(wines_w$ratio.sulfur.dioxide)
p2 <- p2 + geom_hline(aes(yintercept = 0.2717, linetype="cut off"), 
                      color = "blue")
p2 <- p2 + scale_linetype_manual("cut off        ",values=c("dotted"),
                        guide=guide_legend(override.aes =
                                             list(colour = c("blue"))))
p2 <- p2 + guides(color = FALSE)

p3 <- plot_vs_quality(wines_w$density)
p3 <- p3 + geom_hline(yintercept = 0.9918, color = "blue",
                      linetype="dotted")
p3 <- p3 + ylab("chlorides [g / cm^3]")

p4 <- plot_vs_quality(wines_w$alcohol)
p4 <- p4 + geom_hline(aes(yintercept = 11.0, linetype=" "), color = "blue")
p4 <- p4 + scale_linetype_manual("cut off",values=c("dotted"),
                        guide=guide_legend(override.aes =
                                             list(colour = c("blue"))))
p4 <- p4 + guides(fill = FALSE, linetype = FALSE )
p4 <- p4 + ylab("alcohol [%]")

g <- ggplotGrob(p2+theme(legend.position="right"))$grobs
l1 <- g[[which(sapply(g, function(x) x$name) == "guide-box")]]


g <- ggplotGrob(p4+theme(legend.position="right"))$grobs
l2 <- g[[which(sapply(g, function(x) x$name) == "guide-box")]]


grid.arrange(arrangeGrob(p1,p2, l1, p3, p4, l2,
    ncol=3,widths=c(3/7,3/7,1/7)), top = "Scatter Plots by Quality")
```
...
```{r echo=FALSE}
ggplot(aes(y = baf_wines_w$residual.sugar, x= rating, color = rating),
       data = baf_wines_w)+
  geom_boxplot(alpha = 0.5)+
  ylab("residual.sugar [g / dm^3]")+
  stat_summary(fun.y=mean, color = "red", geom="point", size = 2,
               aes(fill = "mean"))+
  stat_summary(fun.y=median, color="orange", geom="point", size=2,
               aes(fill = "2Q"))+
  stat_summary(fun.y=qwrap, fun.args = list(c(0.25)),
               color="yellow", geom="point", size=2,
               aes(fill = "1Q"))+
  ...
  facet_wrap(~filtering)+
  geom_hline(aes(linetype="> medium", yintercept = 2.6), color ="blue")+
  geom_hline(aes(linetype="> good", yintercept = 4.212), color ="green")+
  scale_linetype_manual("keep wines",values=c("dashed", "dotted"),
                        guide=guide_legend(override.aes = list(
                          colour = c("green", "blue"))))+
  ggtitle("Impact of Filtering")+
...
```{r echo=FALSE, Plot_Three}
  ...
  p1 <- p1 + annotate("rect", xmin = 4, ymin = 70, xmax = 6, ymax = 150,
                      fill = "yellow", alpha = 0.2)

  p1 <- p1 + annotate("rect", xmin = 7, ymin = 75, xmax = 9, ymax = 130,
                      fill = "yellow", alpha = 0.2)
  ...
  p1 <- p1 + geom_point(aes(color = rating, alpha = rating, size = alcohol,
               ...
               scale_alpha_manual(values = c(0.7,0.2,0.4,0.7))+
               scale_size(range = c(1,6))+
               labs(shape="pH (level)", size ="alcohol [%]")+
               scale_color_brewer(palette = "Blues")+
               theme_dark()
  ...
  p1 + ggtitle("Focus on Excellence")+
    theme(plot.title = element_text(hjust = 0.5))
```
